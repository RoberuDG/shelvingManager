...
class DatabaseUtils:
    ...
    def get_room(room_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM rooms WHERE room_id = {room_id} AND is_active = TRUE''', room_id)

    def get_shelving(shelving_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM shelvings WHERE id = {shelving_id} AND is_active = TRUE''', shelving_id)

    def get_shelve(shelve_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM shelves WHERE shelve_id = {shelve_id} AND is_active = TRUE''', shelve_id)

    def get_item_type(item_type_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM item_types WHERE item_type_type_id = {item_type_type_id} AND is_active = TRUE''', item_type_id)

    def get_item(item_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM items WHERE item_id = {item_id} AND is_active = TRUE''', item_id)

    def get_all_rooms(cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM rooms WHERE is_active = TRUE ORDER BY id''')

    def get_all_shelvings(cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM shelvings WHERE is_active = TRUE ORDER BY id''')

    def get_all_shelves(cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM shelves WHERE is_active = TRUE ORDER BY id''')

    def get_all_item_types(cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM shelving_types WHERE is_active = TRUE ORDER BY id''')

    def get_all_items(cur: Cursor) -> Cursor:
        return cur.execute('''SELECT * FROM items WHERE is_active = TRUE ORDER BY id''')

        # *Métodos de lectura de objetos
    def get_room_object(room_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT name, description, positions FROM rooms WHERE room_id = {room_id} AND is_active = TRUE''', room_id)

    def get_shelving_object(shelving_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT room_id, code, positions FROM shelves WHERE shelving_id = {shelving_id} AND is_active = TRUE''', shelving_id)

    def get_shelve_object(shelve_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT shelving_id, code, order FROM shelves WHERE shelve_id = {shelve_id} AND is_active = TRUE''', shelve_id)

    def get_item_type_object(item_type_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT name, description, positions FROM item_types WHERE item_type_type_id = {item_type_type_id} AND is_active = TRUE''', item_type_id)

    def get_item_object(item_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT shelve_id, item_type_id, position, name, description FROM items WHERE item_id = {item_id} AND is_active = TRUE''', item_id)
    ...
    def get_all_rooms_object(room_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT name, description, positions FROM rooms WHERE is_active = TRUE''', room_id)

    def get_all_shelvings_object(shelving_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT room_id, code, positions FROM shelves WHERE is_active = TRUE''', shelving_id)

    def get_all_shelves_object(shelve_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT shelving_id, code, order FROM shelves is_active = TRUE''', shelve_id)

    def get_all_item_types_object(item_type_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT name, description, positions FROM item_types is_active = TRUE''', item_type_id)

    def get_all_items_object(item_id: int, cur: Cursor) -> Cursor:
        return cur.execute('''SELECT shelve_id, item_type_id, position, name, description FROM items WHERE is_active = TRUE''', item_id)
    ...
    # *Inserción de datos
    def insert_room(room: Habitacion, cur: Cursor) -> bool:
    ...
    def update_room(room_id: int, room: Habitacion, cur: Cursor) -> bool:
    ...
    def deactivate_room(room_id: int, cur: Cursor) -> bool:
        cur.execute(
            '''UPDATE room SET is_active = FALSE WHERE id= {room_id}''', room_id)
        return True if cur.lastrowid > 0 else False

    def deactivate_shelving(shelving_id: int, cur: Cursor) -> bool:
        cur.execute(
            '''UPDATE shelving SET is_active = FALSE WHERE id= {shelving_id}''', shelving_id)
        return True if cur.lastrowid > 0 else False

    def deactivate_shelve(shelve_id: int, cur: Cursor) -> bool:
        cur.execute(
            '''UPDATE shelve SET is_active = FALSE WHERE id= {shelve_id}''', shelve_id)
        return True if cur.lastrowid > 0 else False

    def deactivate_item_type(item_type_id: int, cur: Cursor) -> bool:
        cur.execute(
            '''UPDATE item_type SET is_active = FALSE WHERE id= {item_type_id}''', item_type_id)
        return True if cur.lastrowid > 0 else False

    def deactivate_item(item_id: int, cur: Cursor) -> bool:
        cur.execute(
